...
| `src/ui/` | components: `App.js` (`B5`, every tab — 6.700 lines, the next thing to split), `Inicio.js` (`j5`, onboarding), `Raiz.js` (`w5`), and one file per piece: `ejercicio.js` (`Is`), `cuerpo.js` (`g5`), `constancia.js`, `calentamiento.js` (also `sdcPasoVista`, shared with stretching), `animo.js`, `tarjetas.js` (`ge`), `metronomo.js`, `descanso.js`, `prueba.js` (also `pitido`, the beep), `neuromotor.js`, `iconos.js`… |
...
| `src/main.js` | mounts `w5` and registers the service worker in production |
...
`ue` became `fechaHoy`, not `hoy`, because `sdcDiaPasado` has a local `hoy` that would have shadowed it; the rename tool refuses exactly that. Still minified: the UI components (`B5`, `Is`, `ge`, …), which the JSX pass will rename, and `B5`'s locals (`Ne`, `De`, `B`, `J`, `a`, `o`), which go with splitting `B5`.
...
`diasConstancia` now drops `empty` days **from the front only**, and the legend filters to the statuses actually present (plus "Hoy" for `pending`, and "Sin registro" when a gap really is in view — `h5` items take an optional `borde` so those two near-black swatches are distinguishable). Day one: 1 square, 1 legend entry, card 326→220 px. A player returning after 40 idle days gets one square too, which is the truth.
