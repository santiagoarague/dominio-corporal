...
| `src/ui/` | JSX components: `App.jsx` (`App`, every tab — 5.750 lines, the next thing to split), `Inicio.jsx` (`Inicio`, onboarding), `Raiz.jsx` (`Raiz`), and one file per piece: `ejercicio.jsx` (`FilaEjercicio`), `cuerpo.jsx` (`FiguraCuerpo`), `constancia.jsx`, `calentamiento.jsx` (also `PasoGuiado`, shared with stretching), `animo.jsx`, `tarjetas.jsx` (`Plegable`), `metronomo.jsx`, `descanso.jsx`, `prueba.jsx` (also `pitido`, the beep), `neuromotor.jsx`, `iconos.jsx`; `pantalla.js` (wake lock) has no markup |
...
**React is pinned to 19.2.5**, the exact version the original bundle carried. The UI is **JSX with the automatic runtime** (Vite's default for `.jsx`), and hooks are plain imports: `import { useState, useEffect } from "react"`. Two elements are still `createElement` calls on purpose, because their type arrives as a lowercase parameter (`BotonSistema`'s `icon`, one in `App`) and JSX would read `<e>` as an HTML tag.
