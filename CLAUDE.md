...
| path | what it is |
|---|---|
| `src/datos/` | tables: `ejercicios.js` (`by`, `F2`, `P2`, `W2`, `ra`), `logros.js` (`Jo` and `da`), `guia.js` (`j2`, skills `El`), `salud.js` (`H2`, `X2`, `Dy`, neuromotor), `rangos.js` (`ve`, `au`, titles) |
| `src/logica/` | rules without UI: `partida.js` (`n5`, `ei`, `i5`, `Ea`, undo, missions, Umbral), `rutina.js` (`Md`, `_d`, `jd`, `Oy`, `li`, dates), `extras.js` (primeras veces, streak recount, set marks, gym helpers, `sdcTier`), `tienda.js`, `estiramiento.js`, `combate.js`, `explorar.js`, `primal.js` (also `Dd`, the day hash), `sistemas.js`, `series.js`, `atributos.js`, `respaldo.js` |
| `src/ui/` | components: `App.js` (`B5`, every tab — 6.700 lines, the next thing to split), `Inicio.js` (`j5`, onboarding), `Raiz.js` (`w5`), and one file per piece: `ejercicio.js` (`Is`), `cuerpo.js` (`g5`), `constancia.js`, `calentamiento.js` (also `sdcPasoVista`, shared with stretching), `animo.js`, `tarjetas.js` (`ge`), `metronomo.js`, `descanso.js`, `prueba.js` (also `Ie`, the beep), `neuromotor.js`, `iconos.js`… |
