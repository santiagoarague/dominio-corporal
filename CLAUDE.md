...
1. **Before** — `sdcAnimoAntes`, a card at `order:-6` above the warm-up. Shown only on the day's first session, before any set is marked or the warm-up has started, and never blocking: "Hoy no" hides it for the day. Normal or better → the card simply goes away. Low (≤2) → **"¿Y el cuerpo?"**: Cansancio · Músculos cargados · Me duele algo · Bien.
